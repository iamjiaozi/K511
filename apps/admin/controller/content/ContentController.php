...
class ContentController extends Controller
{
    ...
    public function add()
    {
        if ($_POST) {
			...
			$filename = cleanUrlString($filename);
            $author = post('author');
            $source = post('source');
            ...
            if(isPositiveInteger($filename)){
				alert_back('URL名称不能为纯数字，会导致和ID冲突!');
			}
            ...
            if ($filename) {
                while ($this->model->checkFilename($filename)) {
                    $filename = $filename . '-' . mt_rand(1, 99999999);
                }
            }
                    ...
                    if ($listall) {
                        ...
                        foreach ($listall as $key => $value) {
                            if ($sorting[$key] === '' || ! is_numeric($sorting[$key]))
                                $sorting[$key] = 255;
                            $this->model->modContentpaixu($value, "sorting=" . $sorting[$key]);
                        }
                        $this->log('修改内容排序成功！');
                        success('修改成功！', - 1);
                    } else {
