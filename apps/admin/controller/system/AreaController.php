...
class AreaController extends Controller
{
    ...
    public function add()
    {
        if ($_POST) {
            // 获取数据
            $acode = removePhpCode(post('acode', 'var'));
            $pcode = removePhpCode(post('pcode', 'var'));
            $name = removePhpCode(post('name'));
			$acode = cleanSpWords($acode,true);   
			$name = cleanSpWords($name,true);
    ...
    public function mod()
    {
        ...
        if ($_POST) {
            // 获取数据
            $acode_new = removePhpCode(post('acode', 'var'));
            $pcode = removePhpCode(post('pcode', 'var'));
            $name = removePhpCode(post('name'));
			$acode_new = cleanSpWords($acode_new,true);   
			$name = cleanSpWords($name,true);
            $domain = removePhpCode(post('domain'));
            $is_default = post('is_default');
