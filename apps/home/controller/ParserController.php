...
class ParserController extends Controller
{
    ...
    public function parserAfter($content)
    {
        ...
        $content = $this->parserCommentLabel($content); // 解析评论
		$content = $this->parserMyCommentLabel($content);// 解析我的评论
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if ($page) {
                    ...
                    if (! ! $get_tag = get('tag', 'vars')) {
                        testforbWords($get_tag);
						$get_tag = escape_string($get_tag);
						$get_tag = str_ireplace('+',' ',$get_tag);
                        if ($fuzzy) {
    ...
    public function parserSearchLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
				...
				testforbWords($testForb);
				testforbSearch($testForb);
    ...
    public function parserIfLabel($content)
    {
        ...
        if (preg_match_all($pattern, $content, $matches)) {
            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                if (preg_match('/(\([\w\s\.]+\))|(\$_GET\[)|(\$_POST\[)|(\$_REQUEST\[)|(\$_COOKIE\[)|(\$_SESSION\[)|(file_put_contents)|(file_get_contents)|(fwrite)|(phpinfo)|(base64)|(`)|(shell_exec)|(passthru)|(pcntl_exec)|(popen)|(proc_open)|(print_r)|(print)|(urldecode)|(chr)|(__FILE__)|(__DIR__)|(call_user_)|(preg_replace)|(array_map)|(array_reverse)|(array_filter)|(getallheaders)|(get_headers)|(decode_string)|(htmlspecialchars)|(session_id)|(strrev)|(substr)|(php.info)|(@file.@_put_content)/i', $matches[1][$i])) {
                    $danger = true;
                }
                ...
                //if标签解析
				$matches[1][$i] = str_ireplace(array('\\x','%2F%2F','//','%2F%2A','/%2A','%2F*','/*','%28','%29','%3B','%60','(',')',';','`','.','--','#','require','system','include','request','copy','assert','eval','exec'),'',$matches[1][$i]);
        ...
        if (isset($params['maxwidth']) || isset($params['maxheight'])) {
            ...
            if (! file_exists($max_out_file) && file_exists($max_src_file)) {
                if (resize_img($max_src_file, $max_out_file, $maxwidth, $maxheight)) {
                    $data = str_replace(DOC_PATH, '', $max_out_file);
                }
        ...
        if (isset($params['width']) || isset($params['height'])) {
            ...
            if (! file_exists($out_file) && file_exists($src_file)) {
                if (cut_img($src_file, $out_file, $width, $height)) {
                    $data = str_replace(DOC_PATH, '', $out_file);
                }
            ...
            case 'content':
                ...
                if (! ! $tags = $this->model->getTags()) {
                    ...
                    foreach ($tags as $value) {
                        $data->content = preg_replace('/' . $value->name . '/', '<a href="' . md5($value->link) . '">' . md5($value->name) . '</a>', $data->content, $this->config('content_tags_replace_num') ?: 5);
                    }
                    foreach ($tags as $value) {
						$data->content = str_replace(md5($value->name),$value->name,$data->content);
						$data->content = str_replace(md5($value->link),$value->link,$data->content);
					}
