...
class Mysqli implements Builder
{
    ...
    public function query($sql, $type = 'master', $params = array())
    {
        ...
        //修复：删除全局初始化master，改为分支内按需连接，保证读写分离
        switch ($type) {
            case 'master':
                if (! $this->master) {
                    $cfg = Config::get('database');
                ...
                //修复：主库使用自身连接转义参数
                if (!empty($params)) {
                    $sql = $this->bindParams($sql, $params, $this->master);
                }
            ...
            case 'slave':
                ...
                //修复：从库使用自身连接转义参数
                if (!empty($params)) {
                    $sql = $this->bindParams($sql, $params, $this->slave);
                }
    ...
    // 安全替换参数占位符
    // 修复：入参改为当前数据库连接对象，区分主/从库
    private function bindParams($sql, $params, $dbConn)
    {
        $offset = 0;
        foreach ($params as $param) {
            ...
            if ($pos !== false) {
                if ($param === null) {
                    ...
                    $replacement = "'" . $dbConn->real_escape_string($param) . "'";
                }
