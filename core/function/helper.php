...
function alert_location($info, $url, $status = 0)
{
    ...
    if (Config::get('return_data_type') == 'json' || is_ajax()) {
        ...
        $info = str_replace(['"', '`'], ['&quot;', '&#96;'], clear_html_blank($info));
        $url  = str_replace(['"', '`'], ['&quot;', '&#96;'], $url);
...
function cleanUrlString($str, $ispath=false) {
...
//只保留中文、英文、数字、空格
function cleanSpWords($str, $stoLow = false) {
    $str = htmlspecialchars_decode($str, ENT_QUOTES);
    $str = trim($str);
    $str = preg_replace('/[^\x{4E00}-\x{9FFF}a-zA-Z0-9\s]/u', '', $str);
    $str = preg_replace('/\s+/', ' ', $str); 
	if($stoLow){
		$str = strtolower($str);
	}
...
function testforbSearch($str){
	if (preg_match('/^[a-zA-Z0-9:\/._-]+$/', $str) && strpos($str, '--') === false) {
        return;
    }
...
function testforbsql($str){
	$testwords = array('--',"\r","\n","\r\n",'%7B','%7D','{','}','%5B','%5D','[',']','%28','%29','(',')','%27',"'",'%2A','*','%3C','%3E','<','>','%3B',';','%22','"','%60','`','\\x');
	foreach($testwords as $value){
		if(stripos($str,$value) !== false){
			alert_back('用户名错误！');
			exit;
