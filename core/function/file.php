...
function handle_upload($file, $temp, $array_ext_allow, $max_width, $max_height, $watermark)
{
    ...
    'bat', 'vbs', 'phtml', 'class',
    'php2', 'php3', 'php4', 'php5',
    'pht', 'phar', 'php7', 'phps', 'shtml', 'cgi',
    'htaccess', 'user.ini',
    'php8', 'php9', 'aspx', 'asmx', 'ashx', 'war',
    'jar', 'py', 'rb', 'pl', 'ini', 'conf', 'sql'
	);
...
function cut_img($src_image, $out_image = null, $new_width = null, $new_height = null, $img_quality = 90)
{
    ...
    switch ($type) {
        case 1:
            imagegif($target_img, $out_image, $img_quality);
            break;
        case 2:
            imagejpeg($target_img, $out_image, $img_quality);
            break;
        case 3:
            imagepng($target_img, $out_image, $img_quality / 10); // $quality参数取值范围0-99 在php 5.1.2之后变更为0-9
            break;
        default:
            imagejpeg($target_img, $out_image, $img_quality);
    }
